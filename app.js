import {defaultModel} from './model-default.js?v=20260918-voice3';
import {selectRoleVoice} from './role-voice.js';
import {AudioCapture} from './audio-capture.js';
const audioCapture=new AudioCapture();
import {Conversation} from './conversation.js?v=20260918-voice3';
import {AuthSession,authMessage} from './auth-session.js';
import { API_BASE } from './config.js';
import { Voice } from './voice.js?v=20260918-voice3';
...
},()=>({playbackReleaseMs:(/iPad|iPhone|iPod/.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1))?3500:300,prepareAudio:state.phone?.audioConsent?()=>audioCapture.start():null,finishAudio:()=>audioCapture.finish(),cancelAudio:()=>audioCapture.cancel(),voiceURI:currentRoleVoice().voice?.voiceURI||state.voiceURI,rate:state.rate,onText:text=>{const el=$('#message');if(el)el.value=text;}}),message=>{state.error=message;voice.status=message;voice.onState?.();const status=$('#operation-status');if(status)status.innerHTML=operationError();toast(message);});
...
document.addEventListener('click',async e=>{
  ...
  try {
    ...
    if(action==='reconnect-mic'&&!state.busy&&!state.phone?.ended&&requireVoiceConsent()){state.error='';const status=$('#operation-status');if(status)status.innerHTML='';conversation.reconnect();}
